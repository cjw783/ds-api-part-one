...
class SWCConfig:
    ...
    def __init__(self, swc_base_url: str = None, backoff: bool = True, backoff_max_time: int = 30, bulk_file_format: str = "csv"):
        """
        설정 클래스의 생성자

        이 생성자는 SWC SDK 클라이언트를 설정할 때 사용할 인자를 받아 내부 변수로 초기화된다. 인자가 전달되지 않으면 기본값 또는 환경변수를 사용한다.
        
        인수: 
        - swc_base_url (선택): 모든 API 호출에 사용할 기본 URL. 직접 전달하거나 환경 변수(SWC_API_BASE_URL)로 지정 가능. 
        - backoff: 오류 발생 시 SDK가 백오프 전략으로 재시도할지 여부를 설정하는 불린 값
